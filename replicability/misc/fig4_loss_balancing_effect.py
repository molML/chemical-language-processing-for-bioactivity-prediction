...
def compute_mean_std_bacc(hp_results):
    baccs = []
    for setup_idx in range(5):
        baccs.append(
            hp_results["misc"]["test_scores"][f"setup-{setup_idx}"]["balanced_accuracy"]
        )

    return np.mean(baccs), np.std(baccs)
...
def find_best_hp_combination(results):
    ...
    for combination_idx in range(n_hps):
        ...
        if combination_val_loss < best_val_loss:
            ...
            if "test_balanced_accuracy_mean" not in hp_results["misc"]:
                best_test_bacc, best_test_bacc_std = compute_mean_std_bacc(hp_results)
            else:
